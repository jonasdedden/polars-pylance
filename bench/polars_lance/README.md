...
BENCH_PYTHON=$(which python) uv run bench/polars_lance/run_matrix.py 2000000,4000000,8000000 55 8
uv run bench/polars_lance/analyse.py "$BENCH_ROOT/results.jsonl"
```
...
The sharded end-to-end pipelines also run distributed -- same size ladder,
six backends (`threads`, `daft`, `ray-core`, `dask`, `ray-data`, `daft-ray`).
See [the dataframe benchmarks guide](../dataframe/README.md), which compares dataframe engines on one node and, at larger sizes, on many.

## Plots

```sh
uv run --group bench bench/polars_lance/plot.py bench/polars_lance/results-m8id4xl.jsonl \
    --out bench/polars_lance/plots
```
...
- **Download and open locally.** `curl -LO` the raw file, or clone the repo and
  open `bench/polars_lance/plots/scaling.html`. The CDN build needs a network; use
...
- **Proxy it.** Paste the file's GitHub URL into
  ...
  `https://htmlpreview.github.io/?https://github.com/jonasdedden/polars-lance/blob/main/bench/polars_lance/plots/scaling.html`
